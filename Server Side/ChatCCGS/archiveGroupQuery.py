...
#http://tartarus.ccgs.wa.edu.au/~1022309/cgibin/ChatCCGS/archiveGroupQuery.py?username=120&password=password123&groupID=10EN1&from=2010-01-05%2012:12:12&to=2018-02-01%2012:12:12
...
try:
    ...
    inp["groupID"] = form["groupID"].value
...
if not serverTools.classExists(inp["groupID"]):
    print("602 No Group")
    exit()


if not serverTools.userIsInClass(inp["username"], inp["groupID"]):
...
try:
    ...
    cur.execute("SELECT * FROM Archived WHERE recipientID=? AND groupID=? AND customGroup=0 AND datestamp BETWEEN ? AND ?",t)

    forUser = cur.fetchall()
    
    t = (inp["username"], inp["groupID"], inp["from"], inp["to"])
    cur.execute("SELECT * FROM Archived WHERE authorID=? AND groupID=? AND customGroup=0 AND datestamp BETWEEN ? AND ?",t)
    fromUser = cur.fetchall()

    t = (inp["username"], inp["groupID"], inp["from"], inp["to"])
    cur.execute("SELECT * FROM Messages WHERE authorID=? AND groupID=? AND customGroup=0 AND datestamp BETWEEN ? AND ?",t)
...
data = forUser + serverTools.removeDuplicates(fromUser + fromUserNonPulled)
